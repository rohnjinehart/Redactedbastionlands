...
export class TalismanWindow extends Application {
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      template: 'systems/bastionlands/templates/talisman-window.hbs',
  ...
  getData() {
    return {
      talismans: game.settings.get('bastionlands', 'globalTalismans'),
      isGM: game.user.isGM,
    };
  }
  ...
  async _syncLinkedTalismansToHunt() {
    ...
    if (needsUpdate) {
      await game.settings.set('bastionlands', 'currentHunt', updatedHunt);
...
Hooks.once('ready', () => {
  game.socket.on('system.bastionlands', (data) => {
