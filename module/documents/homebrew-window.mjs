...
export class HomebrewWindow extends Application {
    ...
    loadHistory() {
        try {
            const historyData = game.settings.get('bastionlands', 'homebrewHistory');
            return historyData || [];
        } catch (e) {
            return [];
        }
    ...
    loadSettings() {
        try {
            const settingsData = game.settings.get('bastionlands', 'homebrewFolderSettings');
            return settingsData || {
                agendaFolder: '',
                blasphemyFolder: '',
                powerFolder: '',
    ...
    saveSettings() {
        try {
            game.settings.set('bastionlands', 'homebrewFolderSettings', this.settings);
        } catch (e) {
    ...
    saveHistory() {
        try {
            game.settings.set('bastionlands', 'homebrewHistory', this.history);
        } catch (e) {
    ...
    static get defaultOptions() {
      return mergeObject(super.defaultOptions, {
        ...
        template: 'systems/bastionlands/templates/homebrew-window.hbs',
        width: 1000,
        height: 1000,
        resizable: true,
        tabs: [
