...
class HuntTracker extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  static PARTS = {
    main: {
      template: "systems/bastionlands/module/components/hunt-tracker/hunt-tracker.hbs"
    }
  }
  ...
  async _onFirstRender(context, options) {
    ...
    // Load saved position
    const savedPosition = game.settings.get('bastionlands', 'huntTrackerPosition');
  ...
  _onRender(context, options) {
    ...
    if (phaseSelect) {
      phaseSelect.addEventListener('change', async (event) => {
        ...
        const hunt = foundry.utils.deepClone(game.settings.get('bastionlands', 'currentHunt'));
        hunt.phase = phase;
        await game.settings.set('bastionlands', 'currentHunt', hunt);
        this._emitUpdate();
      });
    }
    ...
    if (position && (position.top !== undefined || position.left !== undefined)) {
      ...
      this._positionSaveTimeout = setTimeout(() => {
        ...
        if (currentPos.top !== undefined && currentPos.left !== undefined) {
          game.settings.set('bastionlands', 'huntTrackerPosition', {
            top: currentPos.top,
  ...
  async _preparePartContext(partId, context) {
    const huntState = game.settings.get('bastionlands', 'currentHunt');
  ...
  _emitUpdate() {
    game.socket.emit('system.bastionlands', { action: 'updateHunt' });
    this.render(true);
  }
  ...
  async _syncToLinkedTalisman(newValue) {
    const hunt = game.settings.get('bastionlands', 'currentHunt');
    if (hunt.linkedTalismanIndex === undefined || hunt.linkedTalismanIndex === null) return;

    const talismans = game.settings.get('bastionlands', 'globalTalismans');
    ...
    await game.settings.set('bastionlands', 'globalTalismans', talismans);

    // Emit update for talismans and re-render TalismanWindow
    game.socket.emit('system.bastionlands', { action: 'updateTalismans' });
      ...
      buttons: {
        start: {
          ...
          callback: async (html) => {
            ...
            await game.settings.set('bastionlands', 'currentHunt', huntState);
            self._emitUpdate();
    ...
    new Dialog({
      ...
      buttons: {
          ...
          callback: async (html) => {
            ...
            // Reset hunt state
            await game.settings.set('bastionlands', 'currentHunt', {
              active: false,
              sinActorId: null,
              sinName: '',
              sinType: '',
