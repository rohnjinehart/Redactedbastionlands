...
class PathosTracker extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  static PARTS = {
    div: {
      template: "systems/bastionlands/module/components/pathos-tracker/pathos-tracker.hbs"
    }
  }
  ...
  async _onFirstRender(context, options) {
    ...
    // Load saved position
    const savedPosition = game.settings.get('bastionlands', 'pathosTrackerPosition');
    ...
    if (position && (position.top !== undefined || position.left !== undefined)) {
      ...
      this._positionSaveTimeout = setTimeout(() => {
        ...
        if (currentPos.top !== undefined && currentPos.left !== undefined) {
          game.settings.set('bastionlands', 'pathosTrackerPosition', {
            top: currentPos.top,
