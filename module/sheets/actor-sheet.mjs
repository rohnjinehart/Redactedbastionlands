...
// Mythic Bastionlands action imports
import { actorSaveAction }        from '../actions/actor-save-action.mjs';
import { actorAttackAction }      from '../actions/actor-attack-action.mjs';
import { actorBlasphemyAction }   from '../actions/actor-blasphemy-action.mjs';
import { actorDomainAction }      from '../actions/actor-domain-action.mjs';
import { actorTakeDamageAction, openTakeDamageDialog }  from '../actions/actor-take-damage-action.mjs';
import { actorVirtueLossAction }  from '../actions/actor-virtue-loss-action.mjs';
...
export class CainActorSheet extends BaseActorSheet {
  sheetConstants = {
    "CATSessionNumbers": ["0", "2", "4", "6", "8", "10", "12"],
  ...
  get template() {
    ...
    return `systems/bastionlands/templates/actor/actor-${this.actor.type}-sheet.hbs`;
  }
  ...
  async getData() {
    ...
    context.config = CONFIG.RB;

    // Pre-resolve sin mark refs (may be compendium UUIDs or world item IDs)
    if (actorData.type === 'character' || actorData.type === 'human') {
      context._resolvedSinMarks = await this._resolveSinMarkRefs(actorData.system.sinMarks ?? []);
    }
    ...
    if (actorData.type == 'opponent') {
      ...
      context.totalArmor = this.actor.system.armor ?? 0;
    }
    ...
    if (actorData.type == 'human') {
      this._prepareItems(context);
      this._prepareHumanData(context);
    }
    ...
    context.globalTalismans = game.settings.get('bastionlands', 'globalTalismans');
  ...
  _prepareCharacterData(context) {
    ...
    context.hasDemonTrait = (context.system.currentAgendaAbilities ?? [])
    ...
    context.currentSinMarks = context._resolvedSinMarks ?? [];
    context.currentSinMarkAbilities = this._getItemsFromIDs(context.system.sinMarkAbilities || []).filter(i => i != null);
    ...
    context.currentSinMarkData = (context.currentSinMarks || []).filter(s => s != null).map(sinMark => {
  ...
  _prepareHumanData(context) {
    // Calculate total armor from equipped items
    const weapons = context.weapons ?? [];
    const armor   = context.armor   ?? [];
    context.totalArmor = [
      ...armor.filter(i => i.system.equipped),
      ...weapons.filter(i => i.system.equipped && i.system.armor > 0),
    ].reduce((sum, i) => sum + (i.system.armor ?? 0), 0);

    // Resolve ability item IDs to world items
    const abilityIds = context.system.abilities || [];
    context.currentAbilities = abilityIds
      .map(id => game.items.get(id))
      .filter(item => item !== undefined);

    // Agenda abilities and blasphemy powers
    context.currentAgendaAbilities = this._getItemsFromIDs(context.system.currentAgendaAbilities || []);
    context.currentBlasphemyPowers = this._getItemsFromIDs(context.system.currentBlasphemyPowers || []);
    context.currentSinMarks = context._resolvedSinMarks ?? [];
  }
  ...
  async _resolveSinMarkRefs(ids) {
    const resolved = await Promise.all(
      ids.map(id =>
        id?.startsWith('Compendium.') ? fromUuid(id) : Promise.resolve(game.items.get(id))
      )
    );
  ...
  _prepareItems(context) {
    ...
    for (let i of context.items) {
      ...
      // Mark equippable items; used in template for the toggle button.
      i.extra = i.extra ?? {};
      i.extra.isEquippable = EQUIPPABLE_TYPES.includes(i.type);

      if (i.type === 'weapon') {
        // Build a compact property string (hefty / long / slow).
        const props = [];
        if (i.system.hefty) props.push('Hefty');
        if (i.system.long)  props.push('Long');
        if (i.system.slow)  props.push('Slow');
        if (i.system.blast) props.push('Blast');
        if (i.system.range) props.push('Range');
        if (i.system.magazine) props.push('Magazine');
        i.extra.propString = props.join(', ');
        weapons.push(i);
      } else if (ARMOR_TYPES.includes(i.type)) {
        armor.push(i);
      } else if (i.type === 'item' || i.type === 'misc') {
        gear.push(i);
      }
    }

    // Total armor from all currently equipped armor items and weapons with an armor value.
    const equippedArmorItems   = armor.filter(i => i.system.equipped);
    const equippedArmorWeapons = weapons.filter(i => i.system.equipped && i.system.armor > 0);
  ...
  _calculateRanges(context) {
    context.items.forEach(item => {
      if (item.system.rangeType && typeof item.system.rangeType === 'string') {
        item.system.rangeValue = this._parseRange(item.system.rangeType);
      }
    });
  }
  ...
  activateListeners(html) {
    ...
    // Scrip tracker +/- buttons and direct input
    html.on('click', '.inv-scrip-dec', () => {
      const cur = this.actor.system.scrip ?? 0;
      this.actor.update({ 'system.scrip': Math.max(cur - 1, 0) });
    });
    html.on('click', '.inv-scrip-inc', () => {
      const cur = this.actor.system.scrip ?? 0;
      this.actor.update({ 'system.scrip': Math.min(cur + 1, 1000) });
    });
    html.on('change', '.inv-scrip-input', (ev) => {
      const val = parseInt(ev.currentTarget.value) || 0;
      this.actor.update({ 'system.scrip': Math.max(0, Math.min(val, 1000)) });
    });

    // Inline range dropdown on weapon rows
    html.on('change', '.item-range-select', (ev) => {
    ...
    html.on('click', '.item-qty-dec', (ev) => {
      const li   = $(ev.currentTarget).closest('[data-item-id]');
      const item = this.actor.items.get(li.data('itemId'));
      if (!item) return;
      const current = item.system.quantity ?? 0;
      item.update({ 'system.quantity': Math.max(0, current - 1) });
    });

    html.on('click', '.item-qty-inc', (ev) => {
      const li   = $(ev.currentTarget).closest('[data-item-id]');
      const item = this.actor.items.get(li.data('itemId'));
      if (!item) return;
      const current = item.system.quantity ?? 0;
      item.update({ 'system.quantity': current + 1 });
    });

    // Inline reload button on weapon rows
    html.on('click', '.item-reload-btn', async (ev) => {
      const li   = $(ev.currentTarget).closest('[data-item-id]');
      const item = this.actor.items.get(li.data('itemId'));
      if (!item || !item.system.magazine) return;

      const max     = item.system.magazineSize ?? 1;
      const current = item.system.magazineCurrent ?? 0;
      const needed  = max - current;

      if (needed <= 0) {
        ui.notifications.info(`${item.name}: magazine already full.`);
        return;
      }

      const ammoId = item.system.ammoItemId;

      if (!ammoId) {
        await item.update({ 'system.magazineCurrent': max });
        ui.notifications.info(`${item.name} reloaded (${max}/${max}).`);
        return;
      }

      const ammoItem = this.actor.items.get(ammoId);
      if (!ammoItem) {
        ui.notifications.warn(`${item.name}: linked ammo not found — unlink and re-link it.`);
        return;
      }
      ...
      if (toUse <= 0) {
        ui.notifications.warn(`No ${ammoItem.name} remaining — cannot reload ${item.name}.`);
        return;
      }

      await Promise.all([
        item.update({ 'system.magazineCurrent': current + toUse }),
        ammoItem.update({ 'system.quantity': available - toUse }),
      ]);

      ChatMessage.create({
        content: `<strong>${item.name}</strong> reloaded — used ${toUse} ${ammoItem.name}. ${available - toUse} remaining in stock.`,
        speaker: ChatMessage.getSpeaker({ actor: this.actor }),
      });
    });

    // Kit item drag-drop onto weapons / armor / gear sections
    html.on('dragover', '.inv-drop-target', (ev) => {
      ev.preventDefault();
      $(ev.currentTarget).addClass('inv-drag-over');
    });
    html.on('dragleave', '.inv-drop-target', (ev) => {
      $(ev.currentTarget).removeClass('inv-drag-over');
    });
    html.on('drop', '.inv-drop-target', async (ev) => {
      ev.preventDefault();
      ev.stopPropagation();
      if (ev.originalEvent.dataTransfer.types.includes('application/bastionlands-reorder')) return;
      const section = $(ev.currentTarget);
      section.removeClass('inv-drag-over');
      const allowedTypes = (section.data('dropTypes') ?? '').split(' ').filter(Boolean);
      let data;
      try {
      ...
      let sourceItem;
      try { sourceItem = await Item.fromDropData(data); } catch (e) { return; }
      if (!sourceItem) return;
      if (!allowedTypes.includes(sourceItem.type)) {
        ui.notifications.warn(`Cannot drop a "${sourceItem.type}" here. Expected: ${allowedTypes.join(', ')}.`);
        return;
      }
      // If the item already lives on this actor, do nothing (no-op move within same sheet)
    ...
    // ---- Agenda Appetites / Aversions ----
    // ---- Agenda Abilities (drop zone) ----
    html.on('click', '.aa-chat-btn', (ev) => {
      const index = parseInt(ev.currentTarget.dataset.index);
      const ability = game.items.get(this.actor.system.currentAgendaAbilities[index]);
      if (!ability) return;
      const catLevel = this.actor.system.CATLEVEL?.value ?? 0;
      const desc = window.formatCatText ? window.formatCatText(ability.system.abilityDescription, catLevel) : ability.system.abilityDescription;
      ChatMessage.create({
        content: `<h3>${ability.system.abilityName}</h3><p>${desc}</p>`,
        speaker: ChatMessage.getSpeaker({ actor: this.actor })
      });
    });

    html.on('click', '.aa-open-btn', (ev) => {
      const itemId = ev.currentTarget.closest('.aa-card').dataset.itemId;
    ...
    html.on('click', '.aa-chat-btn', (ev) => {
      ...
      const item   = game.items.get(itemId);
      if (!item) return;
      const name = item.system.abilityName || item.name;
      const desc = item.system.abilityDescription || '';
      ChatMessage.create({
        speaker: ChatMessage.getSpeaker({ actor: this.actor }),
        content: `
          <div style="background:#1a1a1a;border:1px solid #333;padding:10px;border-radius:5px;color:#ddd;">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
              <img src="${item.img}" style="width:32px;height:32px;border-radius:3px;border:none;object-fit:cover;flex-shrink:0;"/>
              <strong style="color:#fff;font-size:1.05em;">${name}</strong>
            </div>
            ${desc ? `<div style="font-size:0.88em;color:#ccc;">${nl2p(desc)}</div>` : ''}
          </div>`,
      });
    });

    html.on('click', '.aa-remove-btn', async (ev) => {
      const index = parseInt(ev.currentTarget.dataset.index);
      const list = foundry.utils.deepClone(this.actor.system.currentAgendaAbilities ?? []);
      list.splice(index, 1);
      await this.actor.update({ 'system.currentAgendaAbilities': list });
    });

    html.on('dragover', '.aa-dropzone', (ev) => ev.currentTarget.classList.add('dragover'));
    html.on('dragleave drop', '.aa-dropzone', (ev) => ev.currentTarget.classList.remove('dragover'));

    // ---- Blasphemies (drop zone) ----
    html.on('click', '.bl-remove-btn', async (ev) => {
      const index = parseInt(ev.currentTarget.dataset.index);
      const list = foundry.utils.deepClone(this.actor.system.currentBlasphemies ?? []);
      list.splice(index, 1);
      await this.actor.update({ 'system.currentBlasphemies': list });
    });

    // ---- Blasphemy Powers (drop zone) ----
    html.on('click', '.blp-open-btn', (ev) => {
      const itemId = ev.currentTarget.closest('.blp-card').dataset.itemId;
      const item = game.items.get(itemId);
      if (item) item.sheet.render(true);
    });

    // ---- Sin Marks (drop zone) ----
    html.on('click', '.sm-open-btn', async (ev) => {
      const ref = ev.currentTarget.closest('.sm-card').dataset.itemId;
      const item = ref?.startsWith('Compendium.') ? await fromUuid(ref) : game.items.get(ref);
      if (item) item.sheet.render(true);
    });

    // ---- Post trait/power to chat when image is clicked ----
    html.on('click', '.drop-card-icon', (ev) => {
      const card = ev.currentTarget.closest('[data-item-id]');
      if (!card) return;
      const item = game.items.get(card.dataset.itemId);
      if (!item) return;

      let content;
      if (item.type === 'agendaAbility') {
        content = `
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
            <img src="${item.img}" style="width:36px;height:36px;border-radius:3px;object-fit:cover;border:1px solid #333;flex-shrink:0;">
            <strong style="font-size:1.05em;">${item.system.abilityName}</strong>
          </div>
          <hr style="border-color:#333;margin:4px 0;">
          <p style="margin:4px 0;">${item.system.abilityDescription}</p>`;
      } else if (item.type === 'blasphemyPower') {
        const keywords = item.system.keywords?.length
          ? `<div style="font-size:0.8em;color:#bf5fff;margin-bottom:4px;">${item.system.keywords.join(' · ')}</div>`
          : '';
        const passive = item.system.isPassive
          ? `<em style="font-size:0.8em;color:#888;">(Passive)</em> `
          : '';
        content = `
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
            <img src="${item.img}" style="width:36px;height:36px;border-radius:3px;object-fit:cover;border:1px solid #333;flex-shrink:0;">
            <div><strong style="font-size:1.05em;">${item.system.powerName}</strong><br>${passive}<span style="font-size:0.8em;color:#888;">${item.system.blasphemyType}</span></div>
          </div>
          ${keywords}
          <hr style="border-color:#333;margin:4px 0;">
          <p style="margin:4px 0;">${item.system.powerDescription}</p>`;
      } else {
        content = `
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
            <img src="${item.img}" style="width:36px;height:36px;border-radius:3px;object-fit:cover;border:1px solid #333;flex-shrink:0;">
            <strong style="font-size:1.05em;">${item.name}</strong>
          </div>
          <hr style="border-color:#333;margin:4px 0;">
          <p style="margin:4px 0;">${item.system.description ?? ''}</p>`;
      }
    ...
    html.on('click', '.blp-chat-btn', (ev) => {
      const itemId = ev.currentTarget.closest('.blp-card').dataset.itemId;
      const item   = game.items.get(itemId);
      if (!item) return;
      const name = item.system.powerName || item.name;
      const desc = item.system.powerDescription || '';
      const amp  = item.system.ampDescription  || '';
      ChatMessage.create({
        speaker: ChatMessage.getSpeaker({ actor: this.actor }),
        content: `
          <div style="background:#1a1a1a;border:1px solid #440044;padding:10px;border-radius:5px;color:#ddd;">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
              <img src="${item.img}" style="width:32px;height:32px;border-radius:3px;border:none;object-fit:cover;flex-shrink:0;"/>
              <strong style="color:#fff;font-size:1.05em;">${name}</strong>
            </div>
            ${desc ? `<div style="margin-bottom:6px;font-size:0.88em;color:#ccc;">${nl2p(desc)}</div>` : ''}
            ${amp  ? `<div style="margin-top:6px;padding:6px 8px;background:#1a0a1a;border-left:3px solid #cc33ff;border-radius:3px;">
                        <strong style="color:#cc33ff;font-size:0.8em;">SIN AMP</strong>
                        <div style="font-size:0.88em;color:#dda0dd;">${nl2p(amp)}</div>
                      </div>` : ''}
          </div>`,
      });
    });

    html.on('click', '.blp-remove-btn', async (ev) => {
      const index = parseInt(ev.currentTarget.dataset.index);
      const list = foundry.utils.deepClone(this.actor.system.currentBlasphemyPowers ?? []);
      list.splice(index, 1);
      await this.actor.update({ 'system.currentBlasphemyPowers': list });
    });

    html.on('dragover', '.bl-dropzone, .blp-dropzone', (ev) => ev.currentTarget.classList.add('dragover'));
    html.on('dragleave drop', '.bl-dropzone, .blp-dropzone', (ev) => ev.currentTarget.classList.remove('dragover'));

    // ---- Agenda Appetites / Aversions ----
    html.on('click', '.av-add-btn', async (ev) => {
      const field = ev.currentTarget.dataset.field;
      const current = foundry.utils.deepClone(this.actor.system[field] ?? []);
      current.push({ title: '', description: '' });
      await this.actor.update({ [`system.${field}`]: current });
    });

    html.on('click', '.av-delete-btn', async (ev) => {
      const field = ev.currentTarget.dataset.field;
      const index = parseInt(ev.currentTarget.dataset.index);
      const current = foundry.utils.deepClone(this.actor.system[field] ?? []);
    ...
    html.on('click', '.av-edit-btn', (ev) => {
      const card = ev.currentTarget.closest('.av-card');
      if (!card) return;
      card.classList.add('editing');
      card.querySelector('.av-desc-input')?.focus();
    });

    html.on('blur', '.av-desc-input', (ev) => {
      ev.currentTarget.closest('.av-card')?.classList.remove('editing');
    });

    html.on('click', '.av-chat-btn', (ev) => {
      const field = ev.currentTarget.dataset.field;
      const index = parseInt(ev.currentTarget.dataset.index);
      const entry = this.actor.system[field]?.[index];
      if (!entry) return;
      const label = field === 'agendaAppetites' ? 'Appetite' : field === 'agendaAversions' ? 'Aversion' : 'Anima';
    ...
    // ---- Mythic Bastionlands roll buttons ----
    html.on('click', '.rb-save-btn', (ev) => {
      const virtue = ev.currentTarget.dataset.virtue;
      actorSaveAction(this.actor, { virtue });
    });

    html.on('click', '.burnout-recover-btn:not([disabled])', async () => {
      const actor = this.actor;
      if (!actor.system.burnout) return;
      const roll = await new Roll('2d4').roll();
      const updates = { 'system.burnout': false };
      if (actor.system.sinOverflow) {
        const sinVal = actor.system.sinOverflow.value;
        updates['system.sinOverflow.value'] = sinVal + roll.total;
      }
      await actor.update(updates);
      roll.toMessage({
        flavor: `${actor.name} — Sin Raise: +${roll.total} SIN, burnout cleared`,
        speaker: ChatMessage.getSpeaker({ actor })
      });
    });

    html.on('click', '.rb-attack-btn', () => {
      actorAttackAction(this.actor);
    });

    html.on('click', '.rb-blasphemy-btn', () => {
      actorBlasphemyAction(this.actor);
    });

    html.on('click', '.rb-use-domain-btn', () => {
      actorDomainAction(this.actor);
    });

    html.on('click', '.rb-rest-open-btn', () => {
      const actor = this.actor;
      const content = `
        <form class="rb-rest-form">
          <div class="rb-rest-option">
            <label class="rb-rest-label">
              <input type="checkbox" id="rest-brief"/>
              <span class="rb-rest-name">Brief Rest</span>
              <span class="rb-rest-hint">Restore Resolve &amp; clear Burnout</span>
            </label>
          </div>
          <hr class="rb-rest-hr"/>
          <div class="rb-rest-option">
            <label class="rb-rest-label">
              <input type="checkbox" id="rest-body"/>
              <span class="rb-rest-name">Body</span>
              <span class="rb-rest-hint">Restore to max</span>
            </label>
          </div>
          <div class="rb-rest-option">
            <label class="rb-rest-label">
              <input type="checkbox" id="rest-psyche"/>
              <span class="rb-rest-name">Psyche</span>
              <span class="rb-rest-hint">Restore to max</span>
            </label>
          </div>
          <div class="rb-rest-option">
            <label class="rb-rest-label">
              <input type="checkbox" id="rest-virtue"/>
              <span class="rb-rest-name">Virtue</span>
              <span class="rb-rest-hint">Restore to max</span>
            </label>
          </div>
          <div class="rb-rest-option">
            <label class="rb-rest-label">
              <input type="checkbox" id="rest-grace"/>
              <span class="rb-rest-name">Grace</span>
              <span class="rb-rest-hint">Restore to max</span>
            </label>
          </div>
          <hr class="rb-rest-hr"/>
          <div class="rb-rest-option rb-rest-all-row">
            <label class="rb-rest-label">
              <input type="checkbox" id="rest-all"/>
              <span class="rb-rest-name">Restore All</span>
              <span class="rb-rest-hint">Select all options above</span>
            </label>
          </div>
        </form>
        <style>
          .rb-rest-form { background:#1a1a1a; padding:10px 12px; border-radius:4px; display:flex; flex-direction:column; gap:4px; }
          .rb-rest-option { padding:2px 0; }
          .rb-rest-label { display:flex; align-items:center; gap:8px; cursor:pointer; color:#e0e0e0; }
          .rb-rest-label input[type=checkbox] { accent-color:#4CAF50; width:15px; height:15px; cursor:pointer; flex-shrink:0; }
          .rb-rest-name { font-weight:bold; font-size:0.9em; min-width:72px; }
          .rb-rest-hint { font-size:0.78em; color:#888; }
          .rb-rest-hr { border:none; border-top:1px solid #333; margin:4px 0; }
          .rb-rest-all-row .rb-rest-name { color:#4CAF50; }
          .rb-rest-all-row .rb-rest-label input[type=checkbox] { accent-color:#4CAF50; }
        </style>`;
      ...
      new Dialog({
        ...
        buttons: {
          apply: {
            label: '<i class="fas fa-bed"></i> Rest',
            callback: (html) => {
              const el = id => html[0].querySelector(id);
              const update = {};
              if (el('#rest-brief').checked) {
                update['system.resolve.value'] = actor.system.resolve.max;
                update['system.burnout'] = false;
              }
              if (el('#rest-body').checked)   update['system.body.value']    = actor.system.body.max;
              if (el('#rest-psyche').checked) update['system.spirit.value']  = actor.system.spirit.max;
              if (el('#rest-virtue').checked) update['system.virtue.value']  = actor.system.virtue.max;
              if (el('#rest-grace').checked)  update['system.grace.value']   = actor.system.grace.max;
              if (Object.keys(update).length) actor.update(update);
            },
          },
          cancel: { label: 'Cancel' },
        },
        default: 'apply',
        render: (html) => {
          const all      = html[0].querySelector('#rest-all');
          const vitals   = ['#rest-brief','#rest-body','#rest-psyche','#rest-virtue','#rest-grace']
                            .map(id => html[0].querySelector(id));
          all.addEventListener('change', () => {
            vitals.forEach(cb => { cb.checked = all.checked; });
          });
          vitals.forEach(cb => cb.addEventListener('change', () => {
            all.checked = vitals.every(c => c.checked);
          }));
        },
      }, { classes: ['dialog', 'cain'] }).render(true);
    });

    html.on('click', '.rb-take-damage-btn', () => {
      openTakeDamageDialog(this.actor);
    });

    html.on('click', '.rb-virtue-loss-btn', async (ev) => {
      const content = `
        <div>
          <label>Virtue:
            <select id="vl-virtue" style="margin-left:4px;">
              <option value="body">Body</option>
              <option value="spirit">Psyche</option>
              <option value="virtue">Virtue</option>
              <option value="grace">Grace</option>
            </select>
          </label><br/><br/>
          <label>Formula (e.g. d6):
            <input type="text" id="vl-formula" value="d6" style="width:70px;margin-left:4px;"/>
          </label>
        </div>
      `;
      const result = await Dialog.prompt({
        ...
        label: 'Roll',
        callback: html => ({
          virtue: html.find('#vl-virtue').val(),
          amountFormula: html.find('#vl-formula').val() || 'd6',
        }),
      });
      actorVirtueLossAction(this.actor, result);
    });
    ...
    html.find('#add-talisman-button').click(async ev => {
      ...
      const globalTalismans = game.settings.get('bastionlands', 'globalTalismans');
    ...
    html.find('.character-drop-target').on('drop', async event => {
      ...
      let data;
      try { data = JSON.parse(event.originalEvent.dataTransfer.getData('text/plain')); } catch(e) { return; }
      if (!data) return;
      const itemDrop = await Item.fromDropData(data);
      switch(itemDrop.type) {
          ...
          case "misc":
            if (itemDrop.parent?.id !== this.actor.id) {
              try {
                await Item.create(itemDrop.toObject(), { parent: this.actor });
              } catch(err) {
                console.error("Failed to add kit item to actor:", err);
    ...
    // Opponent domain drop zone
    html.find('.opponent-domain-drop').on('dragover', (ev) => {
      ev.preventDefault();
      ev.currentTarget.classList.add('dragover');
    });
    html.find('.opponent-domain-drop').on('dragleave', (ev) => {
    ...
    html.find('.opponent-domain-drop').on('drop', async (event) => {
      ...
      try {
        const data = JSON.parse(event.originalEvent.dataTransfer.getData('text/plain'));
        const itemDrop = await Item.fromDropData(data);
        if (itemDrop.type !== 'domain') {
          ui.notifications.warn('Only domain items can be dropped here.');
          return;
        }
        const currentDomains = foundry.utils.deepClone(this.actor.system.domains || []);
        if (currentDomains.includes(itemDrop.id)) {
          ui.notifications.warn('This domain is already added.');
          return;
        }
        currentDomains.push(itemDrop.id);
        await this.actor.update({'system.domains': currentDomains});
      } catch (err) {
        console.error('Error handling domain drop:', err);
      }
    });

    // Remove domain from opponent
    html.find('.remove-opponent-domain-btn').click(async (ev) => {
      const domainId = ev.currentTarget.dataset.id;
      const currentDomains = foundry.utils.deepClone(this.actor.system.domains || []);
      const updated = currentDomains.filter(id => id !== domainId);
      await this.actor.update({'system.domains': updated});
    });

    // Add weapon row to opponent
    html.on('click', '.opp-weapon-add-btn', async () => {
      const weapons = foundry.utils.deepClone(this.actor.system.weapons || []);
      weapons.push({ name: '', damage: 'd6' });
      await this.actor.update({ 'system.weapons': weapons });
    });

    // Remove weapon row from opponent
    html.on('click', '.opp-weapon-remove-btn', async (ev) => {
    ...
    // Live-update the form class, portrait, and token image when the opponent type dropdown changes.
    // We include system.opponentType in the update so the re-render gets the correct CSS class.
    html.on('change', '.type-select', async (ev) => {
      const form = ev.currentTarget.closest('form');
      if (!form) return;
      const newType = ev.currentTarget.value;

      // Instant CSS feedback before the re-render
      const allTypes = ['ogre', 'idol', 'hound', 'centipede', 'toad', 'lord', 'imago', 'other', 'mother'];
      allTypes.forEach(t => form.classList.remove(t));
      form.classList.add(newType);

      const typeImages = {
        ogre:      'systems/bastionlands/assets/Sins/Ogre.png',
        idol:      'systems/bastionlands/assets/Sins/Idol.png',
        hound:     'systems/bastionlands/assets/Sins/Hound.png',
        centipede: 'systems/bastionlands/assets/Sins/Centipede.png',
        toad:      'systems/bastionlands/assets/Sins/Toad.png',
        lord:      'systems/bastionlands/assets/Sins/lord.png',
        imago:     'systems/bastionlands/assets/Sins/redacted.png',
        other:     'systems/bastionlands/assets/Opponents/drifter.png',
        mother:    'systems/bastionlands/assets/blasphemies/mother.png',
      };

      const img = typeImages[newType];
      if (!img) return;

      // Update the portrait immediately in the DOM for instant visual feedback
      const portrait = form.querySelector('.profile-img');
      if (portrait) portrait.src = img;

      // Persist the image and type together so the re-render is fully consistent
      await this.actor.update({
        img,
        'prototypeToken.texture.src': img,
        'system.opponentType': newType,
      });
    });

    // Click opponent domain icon/name to open item sheet
    html.find('.opponent-domain-item .domain-icon, .opponent-domain-item .domain-name').click(async (ev) => {
      const domainId = ev.currentTarget.closest('.opponent-domain-item').dataset.domainId;
      const item = game.items.get(domainId);
      if (item) item.sheet.render(true);
    });

    // Human NPC ability drop zone
    html.find('.npc-ability-drop-target').on('dragover', (ev) => {
      ev.preventDefault();
      ev.currentTarget.classList.add('dragover');
    });
    html.find('.npc-ability-drop-target').on('dragleave', (ev) => {
      ev.currentTarget.classList.remove('dragover');
    });
    html.find('.npc-ability-drop-target').on('drop', async (event) => {
      event.preventDefault();
      event.currentTarget.classList.remove('dragover');
      try {
        const data = JSON.parse(event.originalEvent.dataTransfer.getData('text/plain'));
        const itemDrop = await Item.fromDropData(data);
        if (itemDrop.type !== 'ability') {
          ui.notifications.warn('Only ability items can be dropped here.');
          return;
        }
        const current = foundry.utils.deepClone(this.actor.system.abilities || []);
        if (current.includes(itemDrop.id)) {
          ui.notifications.warn('This ability is already added.');
          return;
        }
        current.push(itemDrop.id);
        await this.actor.update({ 'system.abilities': current });
      } catch (err) {
        console.error('Error handling ability drop:', err);
      }
    });

    html.find('.human-ability-remove').click(async (ev) => {
      const id = ev.currentTarget.dataset.id;
      const current = foundry.utils.deepClone(this.actor.system.abilities || []);
      await this.actor.update({ 'system.abilities': current.filter(i => i !== id) });
    });

    html.find('.human-ability-icon, .human-ability-name').click((ev) => {
      const id = ev.currentTarget.closest('.human-ability-card').dataset.abilityId;
    ...
    // Drag-to-reorder for all card lists and inventory rows
    this._setupCardReorder(html, '.aa-card',              'system.currentAgendaAbilities');
    this._setupCardReorder(html, '.blp-card',             'system.currentBlasphemyPowers');
    this._setupCardReorder(html, '.sm-card',              'system.sinMarks');
    this._setupCardReorder(html, '.opponent-domain-item', 'system.domains');
    this._setupCardReorder(html, '.opp-weapon-row',       'system.weapons');
    this._setupEmbeddedReorder(html);
}
  ...
  _setupCardReorder(html, cardSel, systemPath) {
    const REORDER_TYPE = 'application/bastionlands-reorder';

    html.on('dragstart', cardSel, (ev) => {
      const card = ev.currentTarget;
      const index = parseInt(card.dataset.index);
      ev.originalEvent.dataTransfer.setData(REORDER_TYPE, JSON.stringify({ systemPath, fromIndex: index }));
      ev.originalEvent.dataTransfer.effectAllowed = 'move';
      card.classList.add('rb-dragging');
    });

    html.on('dragend', cardSel, (ev) => {
      ev.currentTarget.classList.remove('rb-dragging');
    });

    html.on('dragover', cardSel, (ev) => {
      if (!ev.originalEvent.dataTransfer.types.includes(REORDER_TYPE)) return;
      ev.preventDefault();
      ev.currentTarget.classList.add('rb-drag-over');
    });

    html.on('dragleave', cardSel, (ev) => {
      ev.currentTarget.classList.remove('rb-drag-over');
    });

    html.on('drop', cardSel, async (ev) => {
      const raw = ev.originalEvent.dataTransfer.getData(REORDER_TYPE);
      if (!raw) return;
      ev.preventDefault();
      ev.stopPropagation();
      ev.currentTarget.classList.remove('rb-drag-over');
      let reorderData;
      try { reorderData = JSON.parse(raw); } catch(e) { return; }
      if (reorderData.systemPath !== systemPath) return;
  ...
  _setupEmbeddedReorder(html) {
    const REORDER_TYPE = 'application/bastionlands-reorder';

    html.on('dragstart', '.inv-list-row', (ev) => {
      const itemId = ev.currentTarget.dataset.itemId;
      ev.originalEvent.dataTransfer.setData(REORDER_TYPE, JSON.stringify({ type: 'embedded', fromId: itemId }));
      ev.originalEvent.dataTransfer.effectAllowed = 'move';
      ev.currentTarget.classList.add('rb-dragging');
    });

    html.on('dragend', '.inv-list-row', (ev) => {
      ev.currentTarget.classList.remove('rb-dragging');
    });

    html.on('dragover', '.inv-list-row', (ev) => {
      if (!ev.originalEvent.dataTransfer.types.includes(REORDER_TYPE)) return;
      ev.preventDefault();
      ev.currentTarget.classList.add('rb-drag-over');
    });

    html.on('dragleave', '.inv-list-row', (ev) => {
      ev.currentTarget.classList.remove('rb-drag-over');
    });

    html.on('drop', '.inv-list-row', async (ev) => {
      const raw = ev.originalEvent.dataTransfer.getData(REORDER_TYPE);
      if (!raw) return;
      ev.preventDefault();
      ev.stopPropagation();
      ...
      let reorderData;
      try { reorderData = JSON.parse(raw); } catch(e) { return; }
      if (reorderData.type !== 'embedded') return;
      const fromId = reorderData.fromId;
      const toRow = ev.currentTarget;
      const toId = toRow.dataset.itemId;
      if (fromId === toId) return;
      // Collect all rows in this section (same ol.inv-list parent)
      const list = toRow.closest('ol.inv-list');
      if (!list) return;
      const rows = Array.from(list.querySelectorAll('li.inv-list-row[data-item-id]'));
      ...
      const updates = ids.map((id, i) => ({ _id: id, sort: (i + 1) * 100 }));
  ...
  async _rollSinOverflow(actor) {
    ...
    // Calculate new sinOverflow value (overflow handled by preUpdateActor hook)
  ...
  _onDropBlasphemy(event, blasphemy) {
    const list = foundry.utils.deepClone(this.actor.system.currentBlasphemies ?? []);
    if (list.includes(blasphemy.id)) return;
    list.push(blasphemy.id);
    this.actor.update({ 'system.currentBlasphemies': list });
  }
  ...
  async _onDropSinMark(event, sinMark) {
    if (!this.actor || !this.actor.system) return;

    // Store the UUID to keep compendium items out of the world item tab
    const ref = sinMark.uuid ?? sinMark.id;

    const sinMarkList = foundry.utils.deepClone(this.actor.system.sinMarks || []);
    if (sinMarkList.includes(ref)) return;
    sinMarkList.push(ref);

    // Carry over any passive abilities the sin mark already has
    const sinMarkAbilitiesList = foundry.utils.deepClone(this.actor.system.sinMarkAbilities || []);
    const newPassiveAbilities = this._getItemsFromIDs(sinMark.system.abilities || [])
      .filter(ability => ability?.system?.isPassive)
      .map(ability => ability.id);
    const mergedAbilities = sinMarkAbilitiesList.concat(newPassiveAbilities);

    await this.actor.update({
      'system.sinMarks': sinMarkList,
      'system.sinMarkAbilities': mergedAbilities,
    });
  }
  ...
  async _removeSinMarkOrAbility(event) {
    ...
    if (abilityId) {
        ...
        const sinMark = sinMarkId?.startsWith('Compendium.')
          ? await fromUuid(sinMarkId)
          : game.items.get(sinMarkId);

        const newSinMarks = sinMarks.filter(ref => ref !== sinMarkId);
        const newSinMarkAbilities = sinMark
          ? sinMarkAbilities.filter(id => !sinMark.system.abilities.includes(id))
          : sinMarkAbilities;
  ...
  _onSinTypeSelect(sinType) {
    ...
    const sinTypeMapping = {
      ogre: {
        defaultImg: "systems/bastionlands/assets/Sins/ogre.png",
        domains: {
          ability1: {
      ...
      toad: {
        defaultImg: "systems/bastionlands/assets/Sins/toad.png",
        domains: {
          ability1: {
            title: "Hotel for One",
      ...
      idol: {
        defaultImg: "systems/bastionlands/assets/Sins/idol.png",
        domains: {
      ...
      lord: {
        defaultImg: "systems/bastionlands/assets/Sins/lord.png",
        domains: {
      ...
      hound: {
        defaultImg: "systems/bastionlands/assets/Sins/hound.png",
        domains: {
      ...
      centipede: {
        defaultImg: "systems/bastionlands/assets/Sins/centipede.png",
        domains: {
          ability1: {
      ...
      redacted: {
        defaultImg: "systems/bastionlands/assets/Sins/generic_sin.png",
        domains: {
          ability1: {
            title: "The Redacted",
            value: "The Redacted",
