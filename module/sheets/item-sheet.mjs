...
export class CainItemSheet extends BaseItemSheet {
  ...
  /** @override */
  get template() {
    const path = 'systems/bastionlands/templates/item';
  ...
  async getData() {
    ...
    if (this.item.type === "domain") {
      ...
      // Ensure name/img are available at top-level for the template
      context.name = this.item.name;
      context.img  = this.item.img;
    }
    ...
    if (this.item.type === 'weapon' && this.item.system.magazine) {
      const size = this.item.system.magazineSize || 1;
      const current = this.item.system.magazineCurrent ?? 0;
      context.magazinePercent = Math.round((current / size) * 100);
      const ammoId = this.item.system.ammoItemId;
      if (ammoId && this.item.actor) {
        const ammoItem = this.item.actor.items.get(ammoId);
        context.ammoItemName     = ammoItem?.name ?? null;
        context.ammoItemQuantity = ammoItem?.system.quantity ?? 0;
      }
    }

    context.developerMode = game.settings.get('bastionlands', 'developerMode');
    // Add the item's data to context.data for easier access, as well as flags.
    context.system = itemData.system;
    context.flags = itemData.flags;

    // Adding a pointer to CONFIG.RB
  ...
  activateListeners(html) {
    ...
    // Weapon magazine listeners
    if (this.item.type === 'weapon') {
      html.on('click', '.rb-mag-dec', async () => {
        const current = this.item.system.magazineCurrent ?? 0;
        await this.item.update({ 'system.magazineCurrent': Math.max(0, current - 1) });
      });

      html.on('click', '.rb-mag-inc', async () => {
        const current = this.item.system.magazineCurrent ?? 0;
        const max = this.item.system.magazineSize ?? 1;
        await this.item.update({ 'system.magazineCurrent': Math.min(max, current + 1) });
      });

      html.on('click', '.rb-reload-btn', async () => {
        const max     = this.item.system.magazineSize ?? 1;
        const current = this.item.system.magazineCurrent ?? 0;
        ...
        if (!ammoId || !actor) {
          await this.item.update({ 'system.magazineCurrent': max });
          ui.notifications.info(`${this.item.name} reloaded (${max}/${max}).`);
          return;
        }

        const ammoItem = actor.items.get(ammoId);
        if (!ammoItem) {
          ui.notifications.warn('Linked ammo item not found in inventory. Unlink and re-link the ammo item.');
          return;
        }
        ...
        if (toUse <= 0) {
          ui.notifications.warn(`No ${ammoItem.name} remaining — cannot reload.`);
          return;
        }

        await Promise.all([
          this.item.update({ 'system.magazineCurrent': current + toUse }),
          ammoItem.update({ 'system.quantity': available - toUse })
        ]);

        const remaining = available - toUse;
        ChatMessage.create({
          content: `<strong>${this.item.name}</strong> reloaded — used ${toUse} ${ammoItem.name}. ${remaining} remaining in stock.`,
          speaker: actor ? ChatMessage.getSpeaker({ actor }) : undefined
        });
      });

      html.on('click', '.rb-ammo-unlink', async () => {
        await this.item.update({ 'system.ammoItemId': '' });
      });

      html.find('.rb-ammo-drop-target')
        .on('dragover', ev => { ev.preventDefault(); ev.currentTarget.classList.add('dragover'); })
        .on('dragleave drop', ev => ev.currentTarget.classList.remove('dragover'))
        .on('drop', async (event) => {
          event.preventDefault();
          let data;
          try { data = JSON.parse(event.originalEvent.dataTransfer.getData('text/plain')); }
          catch (e) { return; }
          const dropped = await Item.fromDropData(data);
          if (!dropped) return;
          const validAmmoType = dropped.type === 'item' || (dropped.type === 'misc' && dropped.system.isAmmo);
          if (!validAmmoType) {
            if (dropped.type === 'misc' && !dropped.system.isAmmo)
              ui.notifications.warn(`${dropped.name} is not marked as ammo. Enable "Is Ammo" on the item first.`);
            else
              ui.notifications.warn('Only gear items or misc items marked as ammo can be linked as the ammo stock.');
            return;
          }
          await this.item.update({ 'system.ammoItemId': dropped.id });
          ui.notifications.info(`${dropped.name} linked as ammo for ${this.item.name}.`);
        });
    }
  ...
  _onSinTypeSelect(sinType) {
    ...
    switch (sinType) {
      case 'ogre':
        imgPath = 'systems/bastionlands/assets/Sins/ogre.png';
        break;
      case 'lord':
        imgPath = 'systems/bastionlands/assets/Sins/lord.png';
        break;
      case 'centipede':
        imgPath = 'systems/bastionlands/assets/Sins/centipede.png';
        break;
      case 'hound':
        imgPath = 'systems/bastionlands/assets/Sins/hound.png';
        break;
      case 'idol':
        imgPath = 'systems/bastionlands/assets/Sins/idol.png';
        break;
      case 'toad':
        imgPath = 'systems/bastionlands/assets/Sins/toad.png';
        break;
      default:
        imgPath = 'systems/bastionlands/assets/Sins/generic_sin.png';
        break;
    }
  ...
  async _updateObject(event, formData) {
    ...
    // Guard: name must never be undefined (Foundry drops empty strings from formData)
    if (expanded.name === undefined || expanded.name === '') {
      expanded.name = this.item.name;
    }
